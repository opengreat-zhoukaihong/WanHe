   ...
   begin
    ...
    while not Eof do
     begin
      ...
      BaseNode:=AddChild(BSCNode,fieldbyName('BS_NO').Asstring);
...
begin
   ...
   begin
    ...
    begin
     ...
     Sql.add('Where Base_No="'+TreeView1.Selected.text+'"');
     TempName:='CELL.dbf';
    end
    else
    begin
...
begin
   ...
   begin
     ...
     if TempName='BASE.dbf' then  mn:=table1.FieldbyName('BS_NO').Asstring;
...
begin
   ...
   if  TempName ='MSC.dbf'then
    ...
    begin
     ...
     if  TreeView1.Selected.Expanded or not (TreeView1.Selected.HasChildren) then
               BaseNode:=treeview1.items.addchild(TreeView1.Selected,XNO)
            else  BaseNode:=treeview1.items.addchild(TreeView1.Selected.parent,XNO);
     BaseNode.ImageIndex :=BaseNode.level+1;
     BaseNode.selectedindex:=0;
     AddCellNode(XNO);
    end;
...
begin
    ...
    begin
       if TreeView1.Selected.level=4 then
         begin
            table2.fieldbyname('BASE_NO').Asstring:=TreeView1.Selected.parent.Text;
            node:=TreeView1.Selected.parent;
         end
         else
         begin
           node:=TreeView1.Selected;
           table2.fieldbyname('BASE_NO').Asstring:=TreeView1.Selected.Text;
         end;
...
var ssq,mn,mess,text,sq:string;
    node:TTreeNode;
    n:integer;
begin
    State:='Delete';
...
begin
   ...
   begin
      ...
      begin
        close;
        sql.clear;
        sql.Add('delete  from BASE.dbf where BASE_NO="'+delenode.Text+'"');
        execsql;
      end;
   end;
...
begin
 ...
 showmessage(State);
    if TempName='CELL.dbf' then
    begin
     NNO:=table1.FieldbyName('BS_NO').Asstring;
     ssq:='  where BS_NO="'+NNO+'"';
    end;
    if TempName='BASE.dbf' then
    begin
    ...
    if TempName='BSC.dbf' then
    begin
     NNO:=table1.FieldbyName('BSC_NO').Asstring;
     ssq:='  where BSC_NO="'+NNO+'"'
   end;
    if TempName='MSC.dbf' then
    begin
     NNO:=table1.FieldbyName('MSC_NO').Asstring;
     ssq:='  where MSC_NO="'+NNO+'"'
    end;
