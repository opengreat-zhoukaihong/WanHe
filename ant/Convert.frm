...
Dim New_Cell As NewCell1800
Dim New_Ncell As NewCell1800
...
Dim Nortel_BsNo As Long

Sub Ncell_Ericsson()
    Dim s_data As NewCell1800
    Dim gline As String
...
LinMark:
       ...
       Do While Not EOF(2)
          'Seek #2, 962 + pos * 153
          'Seek #2, 1026 + Pos * 309     (31+1)*32+2
          Seek #2, 36 * 32 + 2 + Pos * 341
          Get #2, , s_data
          If UCase(Trim(s_data.bs_no)) = UCase(Trim(bs_no)) Then
             ...
             s_data.time = year(Now) & Format(month(Now), "00") & Format(day(Now), "00")
             Seek #2, 36 * 32 + 2 + Pos * 341
             Put #2, , s_data
             'load_new = load_new + 1
             GoTo s2
    ...
    Do While Not EOF(1)
       ...
       Do While Not EOF(2)
          ...
          If UCase(Trim(s_data.bs_no)) = UCase(Trim(field_data.bs_no)) Then
             ...
             field_data.time = year(Now) & Format(month(Now), "00") & Format(day(Now), "00")
             Seek #3, 36 * 32 + 2 + Pos * 341
             Put #3, , field_data
             Mybs_noTmp = ""
             GoTo s2
          End If
       ...
       Open sinput2 For Binary As #2
       load_new = load_new + 1
       FileNoMatch = FileNoMatch & "        " & newbs_no & Chr(13) & Chr(10)
s2:
       DoEvents
       If Convert_Stop = True Then
          Close
    ...
    Do While Not EOF(7)
       ...
       Do While Not EOF(3)
          'Seek #3, 1026 + Po * 309
          Seek #3, 36 * 32 + 2 + Po * 341
          Get #3, , s_data
          If UCase(Trim(s_data.bs_no)) = UCase(Trim(bs_no)) Then
             'Seek #3, 21 * 32 + 2 + Po * 595
             Seek #3, 36 * 32 + 2 + Po * 341
             pget = 1
             Exit Do
          End If
    ...
    Do While Not EOF(7)
       ...
       scnline = scnline + 1
       If scnline = bs * bline And (ProgressBar1.Value + percent_step) < 99 Then
          ProgressBar1.Value = ProgressBar1.Value + percent_step
          bs = bs + 1
       End If
       Call getbsno(bs_no)
...
'******************************************************************
           ...
           For j = 1 To row
               ...
               If temp_row > 0 Then
                  LinName = mapinfo.eval("temp.cell_name")
                  If InStr(LinName, Chr(0)) > 0 Then
                     LinName = Trim(Left(LinName, InStr(LinName, Chr(0)) - 1))
                  End If
...
'                  radius = Int(radius * 50)
 ...
 '                 radiuslat = radius + lat
                  mapinfo.do " update tch_sts set col1= " + Chr(34) + LinName + Chr(34) + " , lon = " + str(lon) + ",lat = " + str(lat) + ",bearing = " + str(bearing) + " where rowid = " & j
...
'******************************************************
           ...
           For j = 1 To row
               ...
               If temp_row > 0 Then
                  ...
                  lon = lon + 0.0015 * Sin(bearing * 0.01745329252)
                  lat = lat + 0.0015 * Cos(bearing * 0.01745329252)
                  mapinfo.do " update cch_sts set col1= " + Chr(34) + LinName + Chr(34) + " , lon = " + str(lon) + ",lat = " + str(lat) + ",bearing = " + str(bearing) + " where rowid = " & j
                  mapinfo.do "create point into variable sts_mypoint (" & lon & "," & lat & ") symbol(34,7585792,2)"
...
Private Sub Timer1_Timer()
    ...
    Dim NcellFile As Integer
    Dim MyInString As String * 200
    Dim LastOne As String, LastTwo As String
    ...
    If Menu_Flag = 2302 Then
       ...
       If Val(MyRecord.exchange) = 0 Then
         If InStr(UCase(convert_filename(1)), "RLNRP") > 0 Then
          ...
          soutput = Gsm_Path + "\map\" & CellFileName & "1.dbf"
          sinput2 = Gsm_Path + "\map\" & CellFileName & ".dbf"
          
          sinput3 = sinput + "\rlcpp" + "." + Suffix
          sinput4 = sinput + "\rlcfp" + "." + Suffix
          
          If UCase(Dir(sinput1, 0)) <> "" And UCase(Dir(sinput3, 0)) <> "" And UCase(Dir(sinput4, 0)) <> "" Then
         ...
         Else
          ...
          sinput1 = sinput + "\rlnrp" + "." + Suffix
          If Dir(sinput1, 0) <> "" Then
             convert_filename(1) = sinput1
             Ncell_Ericsson
          End If
         End If
          ...
          Unload Me
          If load_new = 0 Then
             MsgBox "CELL库数据已全部更新!" & Chr(10) & Chr(10) & "更新结果保存在User\DownLoad.log", 64, "提示"
          Else
             MsgBox "CELL库数据已更新了" & load_sam & "项!" + Chr(10) + "交换数据中还有" & load_new & "项在CELL库中没有" + Chr(10) + "相同的BS_NO与它们相匹配！" & Chr(10) & Chr(10) & "更新结果保存在User\DownLoad.log", 64, "提示"
          End If
          mapinfo.do "close table cell"
       Else
          ...
          ElseIf Val(MyRecord.exchange) = 5 Then
                ...
                Loop
                LastOne = ""
                Do While Right(InString, 1) <> Suffix
                   LastOne = Right(InString, 1) & LastOne
                ...
                LastTwo = ""
                Do While Right(InString, 1) <> Suffix
                   LastTwo = Right(InString, 1) & LastTwo
                   InString = Left(InString, Len(InString) - 1)
                Loop
                'InString = Left(InString, Len(InString) - 1)
                ...
                '   NcellFile = 1             'ncell
                'End If
                If Len(LastOne) = 2 And Len(LastTwo) = 5 Then
                   NcellFile = 0
                ElseIf Val(LastOne) < 125 And Val(LastTwo) < 125 Then
                   NcellFile = 2
                Else
                   NcellFile = 1
                End If
        ...
        Do While 1
           ...
           Do While Not EOF(2)
              ...
              If Trim(UCase(Old_Cell.bs_no)) = Trim(New_Cell.bs_no) Then
                 ...
                 Old_Cell.time = year(Now) & Format(month(Now), "00") & Format(day(Now), "00")
    ...
    For i = 1 To File_num
       ...
       Do While Not EOF(2)
          ...
          If UCase(Trim(s_data.bs_no)) = UCase(Trim(New_Ncell.bs_no)) Then
             load_sam = load_sam + 1
             For j = 1 To 16
                 s_data.NCELL(i) = New_Ncell.NCELL(i)
             Next
             s_data.time = year(Now) & Format(month(Now), "00") & Format(day(Now), "00")
             'Seek #2, 1026 + Pos * 309
             Seek #2, 36 * 32 + 2 + Pos * 341
...
Sub cell_motorola(sinput1, sinput2) 'Convert program
    ...
    Do While 1
       ...
       For i = 1 To 10
           Call getfield(lines, nonuse)
       Next
       Call getfield(lines, field_data.BASETYPE)
       ...
       NewMicrocell = field_data.BASETYPE
       ...
       Do While Not EOF(2)
          ...
          If s_data.ci = field_data.ci Then
             ...
             field_data.BASETYPE = Trim(NewMicrocell)
             field_data.time = year(Now) & Format(month(Now), "00") & Format(day(Now), "00")
    ...
    For i = 1 To ObjectRecNum
        ...
        If TchFlag Then
           ...
           Do While Not EOF(FileNumber2)
              Seek #FileNumber2, 21 * 32 + 2 + 595 * j
...
Sub CellMotorolaXLS()
    ...
    For i = 1 To XLSRows - 3
        ...
        Do While Not EOF(FileNumber)
           ...
           Seek #FileNumber, 36 * 32 + 2 + CellPos * 341
           ...
           If Trim(XLS_Ci) = Trim(CellData.ci) Then
              ...
              CellData.time = year(Now) & Format(month(Now), "00") & Format(day(Now), "00")
              'For j = 1 To 4
...
Sub UpdateCell(UpdateCellName As String)
    Dim CellData As NewCell1800
    Dim hGsmcellfile As Integer, hDcscellfile As Integer, hCellfile As Integer
    Dim CellHeadData As ScanHead
    Dim OldCellData As Oldcellstru
    Dim OldCellData1 As Oldcellstru1
    Dim OldCellData2 As NewCellStru
    Dim MyMicrocell As String * 1, NonBcchtemp As String
    Dim GsmRecordno As Long
    Dim PerCount As Integer, PerRows As Integer
    Dim i As Integer
    Dim MyCellColor As String
    
    On Error Resume Next
    Label1.Caption = "正在升级 " & UpdateCellName
    Me.Caption = "小区库升级"
    Me.Refresh
    GsmRecordno = 0
    PerCount = 0
    ...
    For i = 1 To 16
        CellData.NCELL(i) = " "
    Next
    If Dir(Left(UpdateCellName, Len(UpdateCellName) - 8) & "cell.old", 0) <> "" Then
       Kill Left(UpdateCellName, Len(UpdateCellName) - 8) & "cell.old"
    End If
    Name UpdateCellName As Left(UpdateCellName, Len(UpdateCellName) - 8) & "cell.old"
    
    hCellfile = FreeFile
    Open Left(UpdateCellName, Len(UpdateCellName) - 8) & "cell.old" For Binary As #hCellfile
    Get #hCellfile, , CellHeadData
    PerRows = Int((CellHeadData.recordno / 100) + 0.5)
    Select Case CellHeadData.RecordLen
        Case 152
            Seek #hCellfile, 930
        Case 153
            Seek #hCellfile, 962
    ...
    hGsmcellfile = funcCreateCell(UpdateCellName)
    Seek #hGsmcellfile, 36 * 32 + 2
    
    For i = 1 To CellHeadData.recordno
        If PerCount = PerRows And ProgressBar1.Value < 90 Then
           ProgressBar1.Value = ProgressBar1.Value + 1
           PerCount = 0
        End If
        ...
        Select Case CellHeadData.RecordLen
            Case 152, 153
                Get #hCellfile, , OldCellData
                If CellHeadData.RecordLen = 153 Then
                   Get #hCellfile, , MyMicrocell
                   MyMicrocell = Format(Val(MyMicrocell))
                Else
                   MyMicrocell = "0"
                End If
            ...
            Case 277
                ...
                CellData.BASETYPE = Format(Val(OldCellData1.microcell))
            ...
            Case 309
                Get #hCellfile, , OldCellData2
        
                CellData.Name = Trim(OldCellData2.Name)
                If InStr(CellData.Name, Chr(0)) > 0 Then
                   CellData.Name = Left(CellData.Name, InStr(CellData.Name, Chr(0)))
                End If
                CellData.bs_no = Trim(OldCellData2.bs_no)
                CellData.ci = Trim(OldCellData2.ci)
                CellData.ARFCN = Trim(OldCellData2.ARFCN)
                CellData.BSIC = Trim(OldCellData2.BSIC)
                CellData.bearing = Trim(OldCellData2.bearing)
                CellData.Lac = Trim(OldCellData2.Lac)
                CellData.downtilt = Trim(OldCellData2.downtilt)
                CellData.MAX_BTS = Trim(OldCellData2.MAX_BTS)
                CellData.MAX_MS = Trim(OldCellData2.MAX_MS)
                CellData.time = Trim(OldCellData2.time)
                CellData.lon = Trim(OldCellData2.lon)
                CellData.lat = Trim(OldCellData2.lat)
                CellData.BASETYPE = Format(Val(OldCellData2.microcell))
                CellData.NONBCCH = Trim(OldCellData2.NONBCCH)
                For j = 1 To 16
                    CellData.NCELL(j) = Trim(OldCellData2.NCELL(j))
                Next
        End Select
'        Debug.Print Trim(CellData.NCELLNAME)
        If Val(CellData.ARFCN) > 124 Then
           CellData.BASETYPE = "3"
        End If
        If CellData.BASETYPE = " " Then
           CellData.BASETYPE = "0"
        End If
        Put #hGsmcellfile, , CellData
        GsmRecordno = GsmRecordno + 1
    Next
    Seek #hGsmcellfile, 5
    Put #hGsmcellfile, , GsmRecordno
    Close
        
        mapinfo.do "Register Table " + Chr(34) + Left(UpdateCellName, Len(UpdateCellName) - 8) & "cell.dbf" + Chr(34) + "Type ""DBF"" Into " + Chr(34) + Left(UpdateCellName, Len(UpdateCellName) - 8) & "cell.tab" + Chr(34)
        mapinfo.do "open table " + Chr(34) + Left(UpdateCellName, Len(UpdateCellName) - 8) & "cell.tab" + Chr(34)
        mapinfo.do "Create Map For cell CoordSys Earth Projection 1, 0 "
        mapinfo.do "fetch first from cell"
        For i = 1 To GsmRecordno
            ...
            If mapinfo.eval("cell.arfcn") > 124 Then
               MyCellColor = Format(MyCellRndColor(Val(mapinfo.eval("cell.arfcn")) Mod 124))
            Else
               MyCellColor = Format(MyCellRndColor(Val(mapinfo.eval("cell.arfcn"))))
            End If
            If Val(mapinfo.eval("cell.basetype")) = 0 Or Val(mapinfo.eval("cell.basetype")) = 3 Then
...
'       finds = InStr(Read_Line, FindChar)
       If finds > 0 Then
          ...
          Read_Line = Right(Read_Line, Len(Read_Line) - finds)
          finds = InStr(Read_Line, FindChar)
          'Nortel_BsNo = Val(Trim(Left(Read_Line, finds - 1)))
          'Read_Line = Right(Read_Line, Len(Read_Line) - finds)
          'finds = InStr(Read_Line, FindChar)
          New_Cell.ci = Trim(Left(Read_Line, finds - 1))
          Read_Line = Right(Read_Line, Len(Read_Line) - finds)
...
'       finds = InStr(Read_Line, FindChar)
       If finds > 0 Then
          ...
          Read_Line = Right(Read_Line, Len(Read_Line) - finds)
          'finds = InStr(Read_Line, FindChar)
          'Nortel_BsNo = CDbl(Trim(Left(Read_Line, finds - 1)))
          For j = 1 To 16
              finds = InStr(Read_Line, FindChar)
              If finds = 0 Then
        ...
        Do While 1
           ...
           Do While Not EOF(2)
              ...
              If Trim(UCase(Old_Cell.bs_no)) = Trim(New_Ncell.bs_no) Then
                 ...
                 End If
                 'If Noflag = Nortel_BsNo Then
                    Old_Cell.ARFCN = New_Ncell.ARFCN
                    Old_Cell.NONBCCH = New_Ncell.NONBCCH
                    Old_Cell.time = year(Now) & Format(month(Now), "00") & Format(day(Now), "00")
                    'Seek #2, 1026 + my_Pos * 309
                    Seek #2, 36 * 32 + 2 + my_Pos * 341
                    Put #2, , Old_Cell
...
'       finds = InStr(Read_Line, FindChar)
       If finds > 0 Then
          ...
          'finds = InStr(Read_Line, FindChar)
          If finds = 0 Then
             New_Ncell.ARFCN = Trim(Read_Line)
             Exit Sub
          Else
             New_Ncell.ARFCN = Trim(Left(Read_Line, finds - 1))
          End If
          New_Ncell.NONBCCH = ""
          For j = 1 To 16
              Read_Line = Right(Read_Line, Len(Read_Line) - finds)
              ...
              If finds = 0 Then
                 If j = 1 Then
                    New_Ncell.NONBCCH = Trim(Read_Line)
                 Else
                    New_Ncell.NONBCCH = Trim(New_Ncell.NONBCCH) & "," & Trim(Read_Line)
                 End If
                 Exit For
              Else
                 If j = 1 Then
                    New_Ncell.NONBCCH = Trim(Left(Read_Line, finds - 1))
                 Else
                    New_Ncell.NONBCCH = Trim(New_Ncell.NONBCCH) & "," & Trim(Left(Read_Line, finds - 1))
                 End If
              End If
          Next
       Else
